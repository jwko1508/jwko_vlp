...
double gpsCBtime_, odom_x_, odom_y_, odom_z_, odom_roll_, odom_pitch_, odom_yaw_, odom_theta_;
...
xytheta_ vehicle_motion_update(xytheta_ particle_pose, float vehicle_speed, float steering_angle, float dt){
    ...
    double speed_noise = sample(pow(fabs(vehicle_speed),velocity_noise_));
...
// covariance calculation
    ...
    bool gps_received_ = true;
    if(ros::Time::now().toSec() - gpsCBtime_ > 1){
        gps_received_ = false;
        ROS_ERROR("NO GPS RECEIVED");
    }

    for(int i=0;i<num_particle_; i++){
        if(i<num_gps_particle_ && gps_received_){
...
// printf("%f\t\t%f\n",road_w_sum,road_w_max);
        ...
        printf("================\n");
        printf("vel_gain: %.2f vel_noise: %.2f\n",velocity_gain_, velocity_noise_);
        printf("rwm:%f vwm:%f\n",road_w_max,vertical_w_max);
        printf("v_g:%.1f v_w:%.0f%%\n",vertical_gain_,
...
// printf("PP\n");
        ...
        std::cout << "MCL dt : " << now_time-curr_t_.toSec() << std::endl;
...
int Init(){
    ...
    pnode_->param<float>("velocity_gain", velocity_gain_, 1.0);
    pnode_->param<float>("velocity_noise", velocity_noise_, 1.0);
    ...
    roadinfo_velodyne_point_sub_ = node_->subscribe<sensor_msgs::PointCloud2>("/vlp_cloud/mcl/roadinfo",1,RoadinfoVelodyneCallback);
    verticalinfo_map_sub_ = node_->subscribe<nav_msgs::OccupancyGrid>("/verticalinfo_map",1, VerticalinfoMapCallback);
    verticalinfo_velodyne_point_sub_ = node_->subscribe<sensor_msgs::PointCloud2>("/vlp_cloud/mcl/verticalinfo",1, VerticalinfoVelodyneCallback);
