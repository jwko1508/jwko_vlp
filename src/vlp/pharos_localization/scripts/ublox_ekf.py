...
class RosGpsEkf:
    ...
    gps_ekf_odom_pub = rospy.Publisher('/odom/ekf', Odometry)
    gps_odom_pub = rospy.Publisher('/onlygps', Odometry)
    ...
    # def gps_predict(self, velo_time):
    ...
    def vehicle_state_callback(self, msg):
        ...
        if msg.state.gear == 2 and abs(self.new_gps[2, 0] - self.old_gps[2, 0]) > 2.0:
            self.back = True
        elif msg.state.gear == 4 and abs(self.new_gps[2, 0] - self.old_gps[2, 0]) > 2.0:
            self.back = False
        p1 = -1.184*pow(10,-5)
        p2 = 0.0003041
        p3 = -0.002027
...
if __name__ == '__main__':
    ...
    gps_ekf.child_frame_id = rospy.get_param('~child_frame_id', '/odom/ekf')
    ...
    rospy.Subscriber('/ebimu/ekf', RPYrate, gps_ekf.rpy_rate_callback, queue_size=1)
