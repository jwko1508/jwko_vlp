...
///전역변수///
double L_PX , L_MX , L_PY ,L_MY , L_PZ, L_MZ;
double R_PX , R_MX , R_PY ,R_MY , R_PZ, R_MZ;
double L_MOVE_Y , R_MOVE_Y;

float L_Xd , L_Yd , L_Zd;
float R_Xd , R_Yd , R_Zd;
double CAR_HEIGHT;
//double PP_ANGLE; /*좌표변환 해줘야 하는 방향과 값*/
//double PM_ANGLE;
double ss;
double ee;
...
public:
    ...
    VLP16_()
    {
        ...
        nh->param<bool>("isTimeLeft",isTimeLeft, true);

        /// *Left
        pnh->param<float>("/pharos_tf_broadcaster_node/L_velodyne_roll",L_Xd, -15.0);
        pnh->param<float>("/pharos_tf_broadcaster_node/L_velodyne_pitch",L_Yd, 0);
        pnh->param<float>("/pharos_tf_broadcaster_node/L_velodyne_yaw",L_Zd, 0.0);

        pnh->param<double>("L_PLUS_X",L_PX, 0);
        pnh->param<double>("L_MINUS_X",L_MX, 0);
        pnh->param<double>("L_PLUS_Y",L_PY, 0);
        pnh->param<double>("L_MINUS_Y",L_MY, 0);
        pnh->param<double>("L_PLUS_Z",L_PZ, 0);
        pnh->param<double>("L_MINUS_X",L_MZ, 0);
        pnh->param<double>("L_MOVE_Y",L_MOVE_Y, 0);

        pnh->param<double>("CAR_HEIGHT",CAR_HEIGHT, 0);
//        pnh->param<double>("PP_ANGLE",PP_ANGLE, 0);
//        pnh->param<double>("PM_ANGLE",PM_ANGLE, 0);
...
//        velodyne_msgs::cpointPtr pt(new velodyne_msgs::cpoint);
...
//        double PP_Z = PP_X * sinf((PP_ANGLE - 15) * M_PI/180) + CAR_HEIGHT;
        ...
        {
            if(fabs(left_input_custom.cpoints[j].y) < 0.03 && left_input_custom.cpoints[j].x > 1 /*&& left_input_custom.cpoints[j].z < PP_Z
            && left_input_custom.cpoints[j].z > PM_Z && left_input_custom.cpoints[j].x < PM_X && left_input_custom.cpoints[j].x > PP_X*/
            && left_input_custom.infos[j].ring == 0)
            {
...
//                }
            ...
            if(fabs(left_input_custom.cpoints[j].y) < 0.03 && left_input_custom.cpoints[j].x < -1.5 && left_input_custom.infos[j].ring == 1)
            {
...
//            CAR_HEIGHT = std_pt->z;
        ...
        if(fabs(alpha_front_L - alpha_back_L) < 2 && alpha_front_L != -100/* && fabs(alpha_pitch_R) < 10*/)
        {
...
//        std::cout << "CAR_HEIGHT_L = "<< CAR_HEIGHT << std::endl;
//        std::cout << "L_Yd_L = "<< L_Yd_L << std::endl;
//        std::cout << "std_pt->z = "<< std_pt->z << std::endl;
//        std::cout << "PM_X = " << PM_X << std::endl;
//        std::cout << "PP_X = " << PP_X << std::endl;
//        std::cout << "PM_Z = " << PM_Z << std::endl;
//        std::cout << "PP_Z = " << PP_Z << std::endl;
...
//        int hou = 0;
//        std::vector<velodyne_msgs::cpoint> a_pt;
//        std::vector<velodyne_msgs::info> a_info;
//        std::vector<velodyne_msgs::cpoint> b_pt;
//        std::vector<velodyne_msgs::info> b_info;
//        double c;
//        double c_angle;
//        std::vector<double> c_angle_vec;
//
//        for (int k = 0; k < left_input_custom.cpoints.size(); k++)
//        {
//            if(hou < left_input_custom.infos[k].hori)
//            {
//                hou = left_input_custom.infos[k].hori;
//            }
//
//        }
//
//        for (int k = 0; k < left_input_custom.cpoints.size(); k++)
//        {
//            if(left_input_custom.infos[k].hori == 0)
//            {
//                a_pt.push_back(left_input_custom.cpoints[k]);
//                a_info.push_back(left_input_custom.infos[k]);
//            }
//
//            else if(left_input_custom.infos[k].hori == hou)
//            {
//                b_pt.push_back(left_input_custom.cpoints[k]);
//                b_info.push_back(left_input_custom.infos[k]);
//            }
//        }
//
//        for (int l = 0; l < a_pt.size(); l++)
//        {
//            for (int i = 0; i < b_pt.size(); i++)
//            {
//                if(a_info[l].ring == b_info[i].ring)
//                {
//                    c = sqrt((a_pt[l].x - b_pt[i].x)*(a_pt[l].x - b_pt[i].x) + (a_pt[l].y - b_pt[i].y)*(a_pt[l].y - b_pt[i].y));
//
//                    c_angle_vec.push_back(c);
//                }
//            }
//        }
//
//
//        if(c_angle_vec.size() == 0)
//        {
//            ROS_WARN("asdasdasdas");
//        }
//
//        for (int m = 0; m < c_angle_vec.size(); m++)
//        {
//            std::cout << c_angle_vec[m] << std::endl;
//        }
...
//        }
        ...
        if(isTimeLeft == true)
        {
            ROS_INFO("set_L time : %lf", diff_value);
        }
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "vlpt_set_L");       //노드명 초기화



    ROS_INFO("started vlpt_set_L");
//    ROS_INFO("SUBTOPIC : ");
//    ROS_INFO("PUBTOPIC : ");
