...
public:
    ...
    vlp_()
    {
        ...
        n->param<bool>("isTimeRight",isTimeRight, false);
...
//        pub_least_full_test.publish(*test_least_full_sensor);
        ...
        if(isTimeRight == true)
        {
            ROS_INFO("remove_R time : %lf", diff_value);
        }
...
{
    ros::init(argc, argv, "vlpt_remove_ground_R");

    ROS_INFO("started vlpt_remove_ground_R");
