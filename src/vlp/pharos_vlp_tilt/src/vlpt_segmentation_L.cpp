...
//        pub_right_pointcloud_seg = nh->advertise<sensor_msgs::PointCloud2>("r_seg",1);
    ...
    std::vector<int> DataInfection(std::vector<int> input_i , pharos_vlp_tilt::perfectarrayPtr &input_my_msg , pharos_vlp_tilt::perfectarrayPtr &object_output/*, double h_min, double h_max*/)
    {
...
//                                object_output->min_center = h_min >= input_my_msg->objects[ way[n] ].info.hori ? PointtoCenter(input_my_msg->objects[ way[n] ].point) : object_output->min_center;
//                                object_output->max_center = h_max <= input_my_msg->objects[ way[n] ].info.hori ? PointtoCenter(input_my_msg->objects[ way[n] ].point) : object_output->max_center;
//
...
//
//                double info_hori_min = 10000;
//                double info_hori_max = -1;
...
//        std::cout<<"함수안에서의 사이즈는안 : "<<all_objects.size()<<std::endl;
    ...
    void LeftSegmentation(const pharos_vlp_tilt::perfectarrayPtr &raw_left_input){
        ...
        for(int i=0; i < objects_vector.size(); i++)
        {
            ...
            for(unsigned int j=0; j < objects_vector[i].objects.size(); j++)
            {
                if(objects_vector[i].objects[j].point.z <=AfterSetHeight)
                {
                    pharos_vlp_tilt::perfect is_z;
                    is_z.point = objects_vector[i].objects[j].point;
                    is_z.state = objects_vector[i].objects[j].state;
                    is_z.info = objects_vector[i].objects[j].info;
                    After_Z_my_msg->objects.push_back(is_z);
                }
            }

//            After_Z_my_msg->min_object = objects_vector[i].min_object;
//            After_Z_my_msg->max_object = objects_vector[i].max_object;
//            After_Z_my_msg->min_center = objects_vector[i].min_center;
//            After_Z_my_msg->max_center = objects_vector[i].max_center;

            if(After_Z_my_msg->objects.size() != 0)
            my_wonderful_output->one.push_back(*After_Z_my_msg);

            for(int k=0; k < objects_vector[i].objects.size(); k++)
            {///요게 한객체 다 하는것
                objects_vector[i].objects[k].point.intensity = my_intensity;
                pcl::PointXYZI pt;
                pt.x = objects_vector[i].objects[k].point.x;
                pt.y = objects_vector[i].objects[k].point.y;
                pt.z = objects_vector[i].objects[k].point.z;
                pt.intensity = objects_vector[i].objects[k].point.intensity;
...
//                std::cout<<"x값나오냐?"<<pt.x<<std::endl;
        ...
        if(isTimeLeft == true)
        {
            ROS_INFO("seg_L time : %lf", diff_value);
        }
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "vlpt_segmentation_L");       //노드명 초기화



    ROS_INFO("started vlpt_segmentation_L");
//    ROS_INFO("SUBTOPIC : ");
//    ROS_INFO("PUBTOPIC : ");
