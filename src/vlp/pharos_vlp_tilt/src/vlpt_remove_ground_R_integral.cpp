        ...
        for (int index = 0; index < num_vec.size(); index++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(pta->objects[num_vec[index]].point.z > pta->objects[num_vec[index - 1]].point.z)
                    {
                        pta->objects[num_vec[index]].state.is_ground = 0; /// 물체이다.
                    }
...
//                    }
                    ...
                    else if(pta->objects[num_vec[index - 1]].point.z > pta->objects[num_vec[index]].point.z)
                    {
                        if((A < Area_Curbstone(pta->objects[num_vec[index - 1]].point, pta->objects[num_vec[index]].point)) * 0.625)
                        {
                            pta->objects[num_vec[index]].state.is_ground = 1; /// 땅이다.
                        }
