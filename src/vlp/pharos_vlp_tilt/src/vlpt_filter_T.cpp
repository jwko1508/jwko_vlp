...
public:
    ...
    vlp_()
    {
        ...
        pnh->param<float>("/pharos_tf_broadcaster_node/T_velodyne_x",T_velodyne_x, 0);
        pnh->param<float>("/pharos_tf_broadcaster_node/T_velodyne_y",T_velodyne_y, 0);
        pnh->param<float>("/pharos_tf_broadcaster_node/T_velodyne_z",T_velodyne_z, 0);
...
//        std::cout<<"odom"<< Odom_.pose.pose.position.x << Odom_.pose.pose.position.y << Odom_.pose.pose.orientation.z<<std::endl;
        ...
        for (unsigned int i = 0; i < input.objects.size(); i++) {
            odomed_data->objects[i].point.x += T_velodyne_x;
            odomed_data->objects[i].point.y += T_velodyne_y;
            odomed_data->objects[i].point.z += T_velodyne_z;
...
//                    isMapInit = false;
//                }
                ROS_ERROR("Map have no data TTT!!!!!!!!!!!!!!!");
            }
...
//        test_cp.header.frame_id="odom";
        ...
        if(isTimeTop == true)
        {
            ROS_INFO("filter_T time : %lf", diff_value);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "vlpt_filter_T");       //노드명 초기화

    ROS_INFO("started vlpt_filter_T");
